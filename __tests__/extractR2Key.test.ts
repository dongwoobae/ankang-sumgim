...
describe("extractR2Key", () => {
  ...
  it("base path(prefix)를 제거한 key를 반환한다", () => {
    expect(extractR2Key("https://pub.example.com/root/photos/1.webp")).toBe("photos/1.webp");
  });

  it("base에 path가 없으면 전체 path를 key로 반환한다", () => {
    process.env.R2_PUBLIC_URL = "https://pub.example.com";
    expect(extractR2Key("https://pub.example.com/photos/1.webp")).toBe("photos/1.webp");
  });
