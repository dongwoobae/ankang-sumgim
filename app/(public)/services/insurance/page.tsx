    ...
    <div>
      ...
      <section className="bg-[#FFFFFF] py-20">
        <div className="max-w-6xl mx-auto px-6">
              ...
              {steps.map((s, i) => (
                <div key={i} className="relative">
                  <div className="bg-[#EEF4FB] border border-[#A8C4E0]/50 rounded-xl p-5">
                    <p className="text-[#1A56A0] text-2xl font-bold mb-2">{s.step}</p>
                    <p
          ...
          {/* 등급 안내 */}
          <div>
            <p className="text-[#1A56A0] text-sm font-semibold tracking-widest mb-2">GRADES</p>
            <h2
              className="text-[#1A2E4A] text-2xl font-bold mb-8"
              style={{ fontFamily: "'Noto Serif KR', serif" }}
            >
              ...
              {grades.map((g, i) => (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-[#1A56A0] text-sm font-bold">{g.score}</span>
                  </div>
                  <p className="text-[#5A7A99] text-sm leading-relaxed">{g.desc}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="bg-[#1A2E4A] py-14">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-start gap-4">
            <Info size={22} className="text-[#2E6DB4] flex-shrink-0 mt-0.5" />
            <div>
              <p
                className="text-[#2E6DB4] font-bold mb-1"
                style={{ fontFamily: "'Noto Serif KR', serif" }}
              >
                등급 신청이 처음이신가요?
              </p>
          ...
          <Link
            ...
            className="flex-shrink-0 flex items-center gap-2 bg-[#1A56A0] text-[#FFFFFF] px-7 py-4 rounded-xl font-bold text-sm hover:bg-[#2E6DB4] hover:text-[#1A2E4A] transition-colors duration-300"
          >
