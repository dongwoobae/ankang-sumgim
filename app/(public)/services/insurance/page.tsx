...
const steps = [
  {
    step: "01",
    title: "신청",
    desc: "국민건강보험공단 지사에 장기요양인정 신청서를 제출합니다.",
  },
  {
    step: "02",
    title: "방문조사",
    desc: "공단 직원이 가정을 방문하여 심신 기능 상태를 조사합니다.",
  },
  {
    step: "03",
    title: "의사소견서",
...
const grades = [
  ...
  },
  {
    grade: "5등급",
    score: "45점 이상",
    desc: "치매 환자로서 치료·보호가 필요한 상태",
  },
  {
    grade: "인지지원등급",
    score: "45점 미만",
    desc: "치매 환자로서 경증 인지기능 저하 상태",
  },
];
    ...
    <div>
      ...
      >
        <div className="max-w-6xl mx-auto px-6">
          ...
          <p className="text-[#5A7A99] mt-3">제도 안내 및 이용 방법</p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center mb-16">
            <div>
              ...
              <p className="text-[#1A2E4A] text-[15px] leading-[1.9] mb-4">
                고령이나 노인성 질병 등으로 일상생활을 혼자 수행하기 어려운
                노인들에게 신체활동·가사활동 지원 등의 서비스를 제공하는
                사회보험 제도입니다.
              </p>
              <p className="text-[#1A2E4A] text-[15px] leading-[1.9]">
                2008년 7월부터 시행된 이 제도는 노인의 심신 기능 회복과 안정적인
                노후 생활을 지원하고, 가족의 돌봄 부담을 사회적으로 분담하는
                것을 목적으로 합니다.
              </p>
            </div>
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div
                  key={i}
                  className="bg-[#EEF4FB] border border-[#A8C4E0]/50 rounded-xl p-5 text-center"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              {steps.map((s, i) => (
                <div key={i} className="relative">
                  <div className="bg-[#EEF4FB] border border-[#A8C4E0]/50 rounded-xl p-5">
                    <p className="text-[#1A56A0] text-2xl font-bold mb-2">
                      {s.step}
                    </p>
                    <p
                  ...
                  {i < steps.length - 1 && (
                    <div className="hidden md:flex absolute top-1/2 -right-2 -translate-y-1/2 z-10">
                      <ArrowRight size={16} className="text-[#1A56A0]" />
                    </div>
                  )}
                </div>
              ))}
      ...
      <section className="bg-[#1A2E4A] py-14">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-start gap-4">
            <Info size={22} className="text-[#E8A020] flex-shrink-0 mt-0.5" />
            <div>
              <p
              ...
              <p className="text-[#5A7A99] text-sm">
                안강 섬김 노인복지센터에서 등급 신청부터 서비스 이용까지 안내해
                드립니다.
              </p>
            </div>
          </div>
          <Link
            ...
            className="flex-shrink-0 flex items-center gap-2 bg-[#1A56A0] text-[#FFFFFF] px-7 py-4 rounded-xl font-bold text-sm hover:bg-[#E8A020] hover:text-[#1A2E4A] transition-colors duration-300"
          >
