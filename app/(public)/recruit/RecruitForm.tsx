        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            ...
            <div className="space-y-5">
              ...
              {[
                { icon: <Briefcase size={18} />, label: "모집 직종", value: "요양보호사" },
                { icon: <MapPin size={18} />, label: "근무 지역", value: "경상북도 경주시 안강읍 및 인근" },
                { icon: <Phone size={18} />, label: "문의 전화", value: "054-763-5988", link: "tel:054-763-5988" },
                { icon: <FileText size={18} />, label: "필요 자격", value: "요양보호사 자격증 소지자\n(경력 무관 · 신입 환영)" },
              ].map((item, i) => (
                <div key={i} className="flex items-start gap-4 p-5 bg-[#EEF4FB] border border-[#A8C4E0]/50 rounded-xl">
                  ...
                  <div>
                    ...
                    {item.link ? (
                      <a href={item.link} className="text-[#1A2E4A] font-medium text-sm hover:text-[#1A56A0] transition-colors whitespace-pre-line">
                        {item.value}
                      </a>
                    ) : (
                      <p className="text-[#1A2E4A] font-medium text-sm whitespace-pre-line">{item.value}</p>
                    )}
                  </div>
                </div>
              ))}
            ...
            <div className="md:col-span-2">
              <h2 className="text-[#1A2E4A] text-xl font-bold mb-6">온라인 입사 지원</h2>

              <form ref={formRef} action={action} className="space-y-5">
                {/* Honeypot */}
                <div aria-hidden="true" style={{ position: "absolute", left: "-9999px", width: "1px", height: "1px", overflow: "hidden" }}>
                  <label htmlFor="website_recruit">Website</label>
                  <input id="website_recruit" name="website" type="text" tabIndex={-1} autoComplete="off" />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div>
                    ...
                    <input name="phone" type="tel" required placeholder="010-0000-0000" value={phone}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {CERTIFICATE_OPTIONS.map((cert) => (
                      <label key={cert} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-[#A8C4E0]/70 bg-[#EEF4FB] cursor-pointer has-[:checked]:bg-[#1A56A0] has-[:checked]:border-[#1A56A0] has-[:checked]:text-white transition-colors">
                        <input type="checkbox" name="certificates" value={cert} className="sr-only" />
                ...
                <div>
                  ...
                  <input name="preferred_region" type="text" required placeholder="예: 안강읍, 강동면"
                    className="w-full px-4 py-3 rounded-xl border border-[#A8C4E0]/70 bg-[#EEF4FB] text-[#1A2E4A] placeholder-[#5A7A99] text-sm focus:outline-none focus:border-[#1A56A0] transition-colors" />
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {WORK_TYPE_OPTIONS.map((opt) => (
                      <label key={opt.value} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-[#A8C4E0]/70 bg-[#EEF4FB] cursor-pointer has-[:checked]:bg-[#1A56A0] has-[:checked]:border-[#1A56A0] has-[:checked]:text-white transition-colors">
                        <input type="radio" name="work_type" value={opt.value} required className="sr-only" />
                ...
                <div>
                  ...
                  <textarea name="introduction" rows={4} maxLength={500} value={intro}
                  ...
                  <p className="text-right text-xs mt-1" style={{ color: intro.length >= 500 ? "#dc2626" : "#5A7A99" }}>
                    ({intro.length}/500)
                  </p>
                </div>

                {state.message && (
                  <div className={`p-4 rounded-xl text-sm font-medium ${state.success ? "bg-[#E8A020]/30 border border-[#1A56A0]/50 text-[#1A2E4A]" : "bg-red-50 border border-red-200 text-red-700"}`}>
                    {state.message}
                  </div>
                )}
