...
// 잘라버리므로, 서버(sharp) 압축/블러에 도달하려면 전송 자체를 그 밑으로 줄여야 한다.
// 서버가 어차피 webp로 재인코딩하므로 작은 파일은 화질 이중 손실을 피해 원본 그대로 보낸다.

const COMPRESS_THRESHOLD = 3.5 * 1024 * 1024;
...
export async function compressImageFile(file: File): Promise<File> {
  ...
  try {
    ...
    const blob =
      (await encodeCanvas(canvas, "image/webp", 0.75)) ??
      (await encodeCanvas(canvas, "image/jpeg", 0.8));
